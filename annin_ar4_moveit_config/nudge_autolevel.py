...
class OneShotJointState(Node):
    def __init__(self, target_names: List[str], timeout: float):
        ...
        self.target_names=target_names; self.timeout=timeout
        self.sub=self.create_subscription(JointState,'/joint_states',self._cb,10)
        self._got=False; self.positions=None
    def _cb(self,msg: JointState):
        m=dict(zip(msg.name,msg.position))
        if all(n in m for n in self.target_names):
            self.positions=[m[n] for n in self.target_names]; self._got=True
    def read_once(self) -> Optional[List[float]]:
        start=self.get_clock().now()
        while rclpy.ok():
            rclpy.spin_once(self,timeout_sec=0.05)
            if self._got: return self.positions
            if (self.get_clock().now()-start).nanoseconds*1e-9 > self.timeout: return None
        return None
...
class JTCClient(Node):
    ...
    def send_abs(self, joint_names: List[str], target_rad: List[float], duration: float):
        traj=JointTrajectory(); traj.joint_names=joint_names
        pt=JointTrajectoryPoint(); pt.positions=target_rad
...
def main():
    ...
    ap.add_argument('--samples', type=int, default=50); ap.add_argument('--warmup-ms', type=int, default=600)
    ap.add_argument('--joints', nargs='+', default=DEFAULT_JOINTS, help='Controller joints in order')
    ap.add_argument('--target-only', nargs='+', default=['joint_3'])
    ap.add_argument('--duration', type=float, default=3.0)
    ap.add_argument('--gain', type=float, default=1.0); ap.add_argument('--invert', action='store_true')
    ap.add_argument('--deadband', type=float, default=0.05); ap.add_argument('--limit', type=float, default=10.0)
    ...
    rclpy.init()
    reader=OneShotJointState(args.joints, args.jstates_timeout)
    cur=reader.read_once(); reader.destroy_node()
    if cur is None:
        rclpy.shutdown(); print("[ERROR] Could not read /joint_states. Is broadcaster active?", file=sys.stderr); sys.exit(2)

    target=list(cur); delta_rad=math.radians(delta)
    for name in args.target_only:
        if name not in args.joints:
            rclpy.shutdown(); print(f"[ERROR] target-only '{name}' not in joints {args.joints}", file=sys.stderr); sys.exit(2)
        idx=args.joints.index(name); target[idx]=cur[idx]+delta_rad
    ...
    node=JTCClient()
    try:
        node.wait(); node.send_abs(args.joints,target,args.duration)
    finally:
