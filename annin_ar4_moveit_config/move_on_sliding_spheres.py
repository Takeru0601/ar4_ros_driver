...
class MoveOnSlidingSphere(Node):
    ...
    def compute_pose_pointing_to_center(self, x, y, z, cx, cy, cz):
        ...
        up = [0, 1, 0]
        x_axis = [up[1]*dir_z - up[2]*dir_y, up[2]*dir_x - up[0]*dir_z, up[0]*dir_y - up[1]*dir_x]
        x_norm = math.sqrt(sum(v**2 for v in x_axis))
        x_axis = [v / x_norm for v in x_axis]

        new_y = [dir_y*x_axis[2] - dir_z*x_axis[1], dir_z*x_axis[0] - dir_x*x_axis[2], dir_x*x_axis[1] - dir_y*x_axis[0]]

        rot_matrix = [[x_axis[0], new_y[0], dir_x], [x_axis[1], new_y[1], dir_y], [x_axis[2], new_y[2], dir_z]]
    ...
    def plan_and_execute_cartesian_path(self):
        ...
        if result and result.error_code.val == 1:
            self.get_logger().info(f'✅ Cartesian path planned with {result.fraction*100:.1f}% success.')
            trajectory = result.solution.joint_trajectory
            from trajectory_msgs.msg import JointTrajectory
            traj_pub = self.create_publisher(JointTrajectory, '/joint_trajectory', 10)
